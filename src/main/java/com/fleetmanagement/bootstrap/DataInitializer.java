 ...
 * - FIXED: Case sensitivity issues resolved
 * 
...
//     private static final String SYSTEM_USER = "SYSTEM";
    ...
    private void performInitialization() {
        ...
        try {
            ...
            // Step 2: Create Roles with validation
            long roleStart = System.currentTimeMillis();
            var roles = createRoles(permissions);
            validateRoleCreation(roles); // ADDED: Validate roles before proceeding
            ...
            log.error("❌ Error during RBAC initialization step", e);
            throw new RuntimeException("RBAC initialization failed in performInitialization", e);
        }
    }

    /**
     * ADDED: Validate that roles were created properly
     */
    private void validateRoleCreation(List<Role> roles) {
        log.info("🔍 Validating created roles:");

        roles.forEach(role -> {
            log.info("   - Role: '{}' (ID: {})", role.getName(), role.getId());
        });
        ...
        if (!hasSuperAdmin) {
            ...
            throw new RuntimeException(
                    "SuperAdmin role was not created properly. Available roles: [" + availableRoles + "]");
        }

        log.info("✅ SuperAdmin role validation passed");
    }
    ...
    /**
     * FIXED: Create individual role with case-aware handling
     */
    private Role createRoleWithPermissions(String roleName, Map<String, Permission> permissionMap) {
        // FIXED: Use case-insensitive search for existing roles
        // The Role entity normalizes names to lowercase, so search with normalized name
        String normalizedSearchName = roleName.toLowerCase();
        var existingRole = roleRepository.findByName(normalizedSearchName);
        ...
        var role = Role.builder()
                .name(roleName) // Keep original case for building, normalize() will handle conversion
   ...
   private List<Permission> getPermissionsForRole(String roleName, Map<String, Permission> permissionMap) {
        List<Permission> rolePermissions = switch (roleName) {
                case "SUPER_ADMIN" -> new ArrayList<>(permissionMap.values()); // All permissions
                ...
                case "TENANT_ADMIN" -> List.of(
                        ...
                        "DEVICE_READ", "DEVICE_REGISTER", "DEVICE_UPDATE", "DEVICE_ASSIGN", "DEVICE_ACTIVATE",
                        "DEVICE_REMOTE_CONFIG", "DEVICE_BULK_OPERATIONS",
                        "VEHICLE_READ", "VEHICLE_CREATE", "VEHICLE_UPDATE", "VEHICLE_DELETE",
                        "VEHICLE_ASSIGN_DEVICE", "FLEET_MANAGE", "VEHICLE_MAINTENANCE",
                        // Location & Tracking
                        "VIEW_LOCATION_LIVE", "VIEW_LOCATION_HISTORY", "EXPORT_LOCATION",
                        "GEOFENCE_MANAGE", "ROUTE_PLANNING", "PLAYBACK_HISTORY",
                        // Alerts & Reports
                        "ALERT_READ", "ALERT_MANAGE", "ALERT_ACKNOWLEDGE", "NOTIFICATION_SEND",
                        "REPORT_VIEW", "REPORT_GENERATE", "REPORT_SCHEDULE",
                        "ANALYTICS_ACCESS", "DATA_EXPORT",
                        // Security
                ...
                ).stream().map(permissionMap::get).filter(Objects::nonNull).collect(Collectors.toList());

                case "DISPATCHER" -> List.of(
                        "VEHICLE_READ", "VIEW_LOCATION_LIVE", "ALERT_READ", "ALERT_ACKNOWLEDGE",
                        "NOTIFICATION_SEND", "EMERGENCY_ALERT", "ROUTE_PLANNING", "REPORT_VIEW"
                ).stream().map(permissionMap::get).filter(Objects::nonNull).collect(Collectors.toList());

                case "INSTALLER" -> List.of(
                        "DEVICE_REGISTER", "DEVICE_ASSIGN", "DEVICE_ACTIVATE",
                        "DEVICE_UPDATE", "VEHICLE_READ", "VEHICLE_ASSIGN_DEVICE"
                ).stream().map(permissionMap::get).filter(Objects::nonNull).collect(Collectors.toList());

                case "VIEWER" -> List.of(
                        "VEHICLE_READ", "VIEW_LOCATION_LIVE", "VIEW_LOCATION_HISTORY",
                        "ALERT_READ", "REPORT_VIEW"
                ).stream().map(permissionMap::get).filter(Objects::nonNull).collect(Collectors.toList());
    ...
    /**
     * FIXED: Create SuperAdmin user with case-insensitive role search
     */
    private User createSuperAdmin(List<Role> roles) {
        ...
        // FIXED: Use case-insensitive search for SuperAdmin role
        var superAdminRole = roles.stream()
                .filter(role -> "SUPER_ADMIN".equalsIgnoreCase(role.getName())) // FIXED: Case-insensitive comparison
                .findFirst()
                .orElseThrow(() -> {
                    ...
                    return new RuntimeException(
                            String.format("SuperAdmin role not found. Available roles: [%s]. " +
                                          "This indicates a case sensitivity issue or role creation failure.", 
                                          availableRoles)
                    );
                });
