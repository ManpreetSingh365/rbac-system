...
public class UserService {
    ...
    public UserResponse createUser(CreateUserRequest request, UUID currentUserId) {
        log.info("Creating user with username: {} by user: {}", request.getUsername(), currentUserId);
        ...
        if (request.getRoleIds() != null && !request.getRoleIds().isEmpty()) {
            Set<Role> validRoles = validateAndGetRoles(request.getRoleIds(), currentUserId);
            user.setRoles(validRoles);
        }
        
        // Step 5: Assign devices if provided
        if (request.getDeviceIds() != null && !request.getDeviceIds().isEmpty()) {
            Set<Device> validDevices = validateAndGetDevices(request.getDeviceIds(), request.getTenantId());
            user.setDevices(validDevices);
        }
        
        // Step 6: Assign vehicles if provided
        if (request.getVehicleIds() != null && !request.getVehicleIds().isEmpty()) {
            Set<Vehicle> validVehicles = validateAndGetVehicles(request.getVehicleIds(), request.getTenantId());
    ...
    public UserResponse getUserById(UUID userId, UUID currentUserId) {
        ...
        validateUserReadPermissions(currentUserId, userId);
    ...
    public Page<UserResponse> getAllUsers(UUID tenantId, UUID currentUserId, Pageable pageable) {
        log.debug("Fetching users for tenant: {} by user: {} with pagination", tenantId, currentUserId);
        ...
        // Apply business logic for user filtering based on current user permissions
        if (permissionService.hasPermission(currentUserId, "SUPER_ADMIN", null)) {
            // SuperAdmin can see all users
            users = tenantId != null ? 
                userRepository.findByTenantIdAndActiveTrue(tenantId, pageable) :
            ...
            users = userRepository.findByTenantIdAndActiveTrue(targetTenantId, pageable);
        }
    ...
    public UserResponse assignDevices(UUID userId, Set<UUID> deviceIds, UUID currentUserId) {
        log.info("Assigning {} devices to user ID: {} by user: {}", deviceIds.size(), userId, currentUserId);
    ...
    private void validateUserReadPermissions(UUID currentUserId, UUID targetUserId) {
        if (!permissionService.hasPermission(currentUserId, "USER_READ", null)) {
